...
export const loadArticles = (limit = 30, page = 1) => async (dispatch) => {
...
export const loadCategory = (category, limit = 10, page = 1) => async (
  dispatch,
) => {
...
export const loadPushArticle = (id) => async (dispatch) => {
  // console.log('GET_PUSH_REQUEST');
  const query = `posts/?include=${id}&_embed`;

  try {
    const {data} = await baseUrlApi.get(query);
    const Catposts = sanitizePost(data[0]);
    // console.log(Catposts);
    await dispatch({
      type: 'push',
      payload: Catposts,
    });
    // console.log('GET_PUSH_SUCCESS');
  } catch (error) {
    // console.log(error);
    dispatch({type: postsTypes.GET_CATEGORY_ERROR, payload: {error}});
    // console.log('GET_PUSH_ERROR');
  }
};
